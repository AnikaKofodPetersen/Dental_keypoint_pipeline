...
def get_vertices_to_delete(ref_mesh, threshold=8.0, radius=4.0):
    # Get all the points from the mesh (assuming it's a numpy array of shape (N, 3))
    points = ref_mesh.points()
    max_z = np.max(points[:, 2])
    threshold_z = max_z - 20
    
    # Build a k-d tree for fast spatial queries
    kdtree = KDTree(points)
    
    # Store vertices to be removed
    vertices_to_remove = []
    instant_remove =  [i for i,p in enumerate(points) if p[2] <= threshold_z]
    vertices_to_remove.extend(instant_remove)
    total = len(points)
    
    # Iterate through each vertex in the mesh
    for i, p in enumerate(points):

        if i not in instant_remove:
            # Periodically print progress (every 100th vertex)
            if i % 100 == 0:
                percent = round(((i + 1) / total) * 100, 2)
                print(f"Cutting mesh: {percent:.2f}%  ", end="\r")
        
            # Find neighbors within the given radius (without storing a full distance matrix)
            neighbors_idx = kdtree.query_ball_point(p, radius)  # Returns indices of points within radius
        
            # Find the local maximum Z-coordinate in the neighborhood (including the point itself)
            # We calculate the max Z value of only the neighboring points
            local_max_z = np.max(points[neighbors_idx, 2])  # Max Z in the neighborhood
        
            # If the Z-coordinate of the current point is lower than the local max by the threshold, mark it for removal
            if local_max_z - p[2] >= threshold:
                vertices_to_remove.append(i)

    return vertices_to_remove

def downsample_mesh(ref_mesh, downsampling_factor=0.01):
    """
    Downsamples the mesh uniformly based on the downsampling factor.
    
    Parameters:
        ref_mesh: The mesh to downsample (assuming it's a numpy array of shape (N, 3)).
        downsampling_factor: The factor by which to downsample (e.g., 0.1 means keeping 10% of the points).
        
    Returns:
        downsampled_points: A numpy array of downsampled mesh points.
        downsampled_idx: The indices of the selected points from the original mesh.
    """
    points = ref_mesh.points()
    total_points = len(points)
    
    # Calculate how many points to select
    num_points_to_select = int(total_points * downsampling_factor)
    
    # Randomly select indices of points for downsampling
    # Uniform sampling without replacement
    downsampled_idx = np.random.choice(total_points, num_points_to_select, replace=False)
    
    # Select the downsampled points using the indices
    downsampled_points = points[downsampled_idx]
    
    return downsampled_points, downsampled_idx

def get_vertices_to_delete(ref_mesh, threshold=8.0, radius=4.0):
    # Get all the points from the mesh (assuming it's a numpy array of shape (N, 3))
    points = ref_mesh.points()
    downsampled_points, downsampled_idx = downsample_mesh(ref_mesh, downsampling_factor=0.01)
    max_z = np.max(points[:, 2])
    threshold_z = max_z - 20
    
    # Get the X and Y coordinates separately (ignoring Z for distance calculation)
    xy_points = downsampled_points[:, :2]
    
    # Build a k-d tree for fast spatial queries on the XY-plane
    ...
    vertices_to_remove = []    
    total = len(points)
    print(total, len(instant_remove))
    ...
    for i, p in enumerate(points):
        if i not in instant_remove:
            ...
            # Find neighbors within the given radius (using the k-d tree)
            neighbors_idx = kdtree.query_ball_point(p[:2], radius)  # Indices of points within the radius

            # Find the local maximum Z-coordinate in the neighborhood (including the point itself)
            local_max_z = np.max(downsampled_points[neighbors_idx, 2])  # Max Z in the neighborhood

            # If the Z-coordinate of the current point is lower than the local max by the threshold, mark it for removal
            if local_max_z - p[2] >= threshold:
                vertices_to_remove.append(i)

    # Mark vertices below threshold_z immediately for removal
    instant_remove = [i for i, p in enumerate(points) if p[2] <= threshold_z]
    vertices_to_remove.extend(instant_remove)
...
def threshold_cut(mesh, threshold=8.0, radius=4.0, teeth_type="man", name="test", out_path="./"):
    o_mesh = orient_mesh(mesh, teeth_type = teeth_type)
    del_ids = get_vertices_to_delete(o_mesh, threshold=threshold, radius=radius)
    if len(del_ids) > 1:
        o_mesh = o_mesh.deletePoints(indices=del_ids)
    o_mesh.clean()
    clean_mesh = Remove_islands(o_mesh)

    # Return results
    vedo.write(clean_mesh,os.path.join(out_path, f"{name}.stl"))
