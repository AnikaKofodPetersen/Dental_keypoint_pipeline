...
void SHOTComputer::getSHOTLocalRF(
  ...
  if (np < 5){
    // AKP
    // Return default output when there are not enough points
    X.x = 1.0; X.y = 0.0; X.z = 0.0; // Default X vector
    Z.x = 0.0; Z.y = 0.0; Z.z = 1.0; // Default Z vector
    Y = cross_product(Z, X);         // Default Y vector as the cross product of Z and X
...
const {
  ...
  if (n_neighs < m_minNeighbors) {
    std::cout << "[WARNING] Neighborhood has less than "
              ...
              << "Setting discriptor to zero (previously 'Aborting description of feature point') " << feat_index
              << std::endl;
    desc.resize(getDescriptorLength(), 0); // Ensure all elements are zero AKP
    return;
  }
